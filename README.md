...
**Blacklist** означает, что токен на заданное время **не участвует** в поиске IP и вообще **не используется** задачами бота — он пропускается, пока не истечёт срок блокировки (запись хранится в `/data/blacklist.json` и переживает перезапуск контейнера).

- Если API вернул **дневной лимит** (`daily limit` / `daily_limit_exceeded`) — токен попадает в чёрный список на **24 часа**.
- Если API сообщил, что **на аккаунте недостаточно средств** / квоты (**нет баланса**, лимит оплаты и т. п.) — на **1 час**.
- В остальных случаях (когда для токена задаётся блокировка) по умолчанию используется **24 часа**, если в коде не указано иное.

После истечения срока токен снова считается доступным и может использоваться как обычно.
